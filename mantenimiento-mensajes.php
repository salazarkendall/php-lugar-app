    ...
    if(isset($_POST['enviar']))
                {
                    ...
                    $sql = "INSERT into mensaje (id_miembro, nombre, fecha, link_pdf)
                        VALUES ('".$_POST["predicador"]."','".$_POST["titulo"]."','".$_POST["fecha"]."','".$_POST["link_pdf"]."')";
...
<body>
    ...
    <!-- Login Modal -->
            ...
            <form action="mantenimiento-mensajes.php" method="POST">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">Insertar mensaje</h4>
                  ...
                  <div class="modal-body mx-3">
                    ...
                    <h5>Predicador</h5>
                    <select class="form-control validate" name="predicador" id="predicador">
                    <?php 
                        $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        $sql="SELECT *,concat(nombre, ' ', apellido1) as nombrePredicador from persona where id_permiso=1";
                        $result=mysqli_query($conexion,$sql);

                        while($mostrar=mysqli_fetch_array($result)){
                        echo '<option value="'.$mostrar['cedula'].'">'.$mostrar['nombrePredicador'].'</option>';
                    }
                     ?>
                     </select>
                     </div>
                    ...
                    <div class="md-form mb-4">
                      <input type="text" id="titulo" name="titulo" class="form-control validate" placeholder="Título del mensaje">
                    </div>

                    <div class="md-form mb-4">
                        <h5>Fecha</h5>
                      <input type="Date" id="fecha" name="fecha" class="form-control validate" placeholder="Fecha" value="<?php echo date('Y-m-d'); ?>">
                    </div>

                    <div class="md-form mb-4">
                      <input type="text" id="link_pdf" name="link_pdf" class="form-control validate" placeholder="Link del PDF">
                    </div>
    ...
    <!-- Eliminar Modal -->
            ...
            <form action="logica/loguear.php" method="POST">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">¿Seguro que desea eliminar?</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
            ...
            <div class="row">
                ...
                <div class="col-12">
                    <div class="section-heading">
                        <h2>Mensajes</h2>
                        <p>Gestioná todos los sermones, charlas y prédicas. </p>
                    </div>
                    <table class="table table-striped">
                        <tr>
                            <td><h5>Código</h5></td>
                            <td><h5>Predicador<h5></td>
                            <td><h5>Título<h5></td>
                            <td><h5>Fecha<h5></td>
                            <td><h5>Link<h5></td>    
                        ...
                        <?php 
                        $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        $sql="SELECT mensaje.*, concat(persona.nombre, ' ', persona.apellido1) as nombrePredicador FROM mensaje INNER JOIN persona ON mensaje.id_miembro = persona.cedula";
                        $result=mysqli_query($conexion,$sql);

                        while($mostrar=mysqli_fetch_array($result)){
                        echo "<tr>";
                        echo "<td><p>".$mostrar['id_mensaje']."<p></td>";
                        echo "<td><p>".$mostrar['nombrePredicador']."<p></td>";
                        echo "<td><p>".$mostrar['nombre']."<p></td>";
                        echo "<td><p>".$mostrar['fecha']."<p></td>";
                        echo "<td><p class='btn btn-light'><a href=".$mostrar['link_pdf'].">Ver PDF</a><p></td>";
                        echo "<td><p class='btn btn-light'><a href=modificarmensaje.php?id_mensaje=".$mostrar['id_mensaje'].">Modificar</a><p></td>";
                        echo "<td><p class='btn btn-light' ><a href=validareliminacion.php?tabla=mensaje&id_mensaje=".$mostrar['id_mensaje'].">Eliminar</a><p></td>";
