...
$id_activo = $_GET['id_activo'];
    ...
    if(isset($_POST['enviar']))
                {
                    $sql = "Update activo set id_miembro='".$_POST["miembro"]."', id_ministerio=".$_POST["ministerio"].",nombre='".$_POST["nombre"]."',cantidad=".$_POST["cantidad"].", descripcion='".$_POST["descripcion"]."',estado='".$_POST["estado"]."' where id_activo='".$_POST["id_activo"]."'";
    ...
    <?php 
            $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        ...
                        while($mostrar=mysqli_fetch_array($result)){
                            ?>
            <div>
            <form action="modificaractivo.php" method="POST">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">Modificar activo</h4>
                  </div>
                  <div class="modal-body mx-3">

                    <div class="md-form mb-4">
                        <h5>Código</h5>
                        <?php echo "<input type='text' id='id_activo' name='id_activo' class='form-control validate' placeholder='Código' value='".$mostrar['id_activo']."' readonly>";?>
                    </div>

                    <div class="md-form mb-4">
                    <h5>Miembro que modifica</h5>
                    <select class="form-control validate" name="miembro" id="miembro" readonly>
                    <?php 
                        $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        $query2="SELECT *,concat(nombre, ' ', apellido1) as nombreMiembro from persona where email='".$emailSesion."'";
                        $result2=mysqli_query($conexion,$query2);

                        while($row=mysqli_fetch_array($result2)){
                        echo '<option value="'.$row['cedula'].'">'.$row['nombreMiembro'].'</option>';
                    }
                     ?>
                     </select>
                     </div>
                    ...
                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='nombre' name='nombre' class='form-control validate' placeholder='Nombre del activo' value='".$mostrar['nombre']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='cantidad' name='cantidad' class='form-control validate' placeholder='Cantidad' value='".$mostrar['cantidad']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='descripcion' name='descripcion' class='form-control validate' placeholder='descripcion' value='".$mostrar['descripcion']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='estado' name='estado' class='form-control validate' placeholder='descripcion' value='".$mostrar['estado']."'>";?>
                    </div>
