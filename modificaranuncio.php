    ...
    if(isset($_POST['enviar']))
                {
                    ...
                    $sql = "Update anuncio set id_miembro='".$_POST["miembro"]."', titulo='".$_POST["titulo"]."',anuncio='".$_POST["anuncio"]."',imagen='".$_POST["imagen"]."' where id_anuncio='".$_POST["id_anuncio"]."'";
    ...
    <?php 
            $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        ...
                        while($mostrar=mysqli_fetch_array($result)){
                            ?>
            <div>
            <form action="modificaranuncio.php" method="POST" enctype="multipart/form-data">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">Modificar anuncio</h4>
                  </div>
                  <div class="modal-body mx-3">

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='id_anuncio' name='id_anuncio' class='form-control validate' placeholder='Código' value='".$mostrar['id_anuncio']."' readonly>";?>
                    </div>

                    <div class="md-form mb-4">
                    <h5>Miembro que anuncia:</h5>
                    <select class="form-control validate" name="miembro" id="miembro">
                    <?php 
                        $query="SELECT *,concat(nombre, ' ', apellido1) as nombreMiembro from persona where id_permiso=1 or id_permiso=2";
                        $resultado=mysqli_query($conexion,$query);

                        while($fila=mysqli_fetch_array($resultado)){
                            if ($fila['cedula']==$mostrar['id_miembro']) {
                                echo '<option value="'.$fila['cedula'].'" selected>'.$fila['nombreMiembro'].'</option>';
                            }
                            else{
                                echo '<option value="'.$fila['cedula'].'">'.$fila['nombreMiembro'].'</option>';
                            }
                    }
                     ?>
                     </select>
                    ...
                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='titulo' name='titulo' class='form-control validate' placeholder='Título' value='".$mostrar['titulo']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='anuncio' name='anuncio' class='form-control validate' placeholder='Descripción del anuncio' value='".$mostrar['anuncio']."'>";?>
                    </div>
                    <?php
                    if (file_exists("img/anuncios/".$mostrar['id_anuncio'].".jpg")) {
                            echo "<h5>Imagen Actual</h5><div class='md-form mb-4'><img src='img/anuncios/".$mostrar['id_anuncio'].".jpg' height='400' width='400'></div>";
                        }
                        else{
                    ...
                    <h5>Seleccionar nueva imagen</h5>
                        <?php echo "<input type='file' id='imagen' name='imagen' class='form-control validate' accept='.jpg'>";?>
                    </div>
