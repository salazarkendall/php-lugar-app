...
$id_ministerio = $_GET['id_ministerio']; 
    ...
    if(isset($_POST['enviar']))
                {
                    $sql = "Update ministerio set id_director='".$_POST["director"]."', nombre='".$_POST["nombre"]."',descripcion='".$_POST["descripcion"]."' where id_ministerio='".$_POST["id_ministerio"]."'";

                    $result = mysqli_query($conn,$sql);
                    $message = 'Datos del ministerio actualizados';
...
<body>
    ...
    <?php 
            $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        $sql="SELECT * from ministerio where id_ministerio=$id_ministerio";
                        $result=mysqli_query($conexion,$sql);

                        while($mostrar=mysqli_fetch_array($result)){
                            ?>
            <div>
            <form action="modificarministerio.php" method="POST">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">Actualizar datos del ministerio</h4>
                  </div>
                  <div class="modal-body mx-3">

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='id_ministerio' name='id_ministerio' class='form-control validate' placeholder='Código' value='".$mostrar['id_ministerio']."' readonly>";?>
                    </div>
                    ...
                    <?php 
                        $query="SELECT *,concat(nombre, ' ', apellido1) as nombreDirector from persona where id_permiso=1 or id_permiso=2";
                        $resultado=mysqli_query($conexion,$query);

                        while($fila=mysqli_fetch_array($resultado)){
                            if ($fila['cedula']==$mostrar['id_director']) {
                                echo '<option value="'.$fila['cedula'].'" selected>'.$fila['nombreDirector'].'</option>';
                            }
                            else{
                                echo '<option value="'.$fila['cedula'].'">'.$fila['nombreDirector'].'</option>';
                            }
                    }
                     ?>
                     </select>
                    ...
                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='nombre' name='nombre' class='form-control validate' placeholder='Nombre del ministerio' value='".$mostrar['nombre']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='descripcion' name='descripcion' class='form-control validate' placeholder='Descripción' value='".$mostrar['descripcion']."'>";?>
                    </div>
