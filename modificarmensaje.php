...
$id_mensaje = $_GET['id_mensaje'];    
    ...
    if(isset($_POST['enviar']))
                {
                    $sql = "Update mensaje set id_miembro='".$_POST["predicador"]."', nombre='".$_POST["titulo"]."',fecha='".$_POST["fecha"]."',link_pdf='".$_POST["link_pdf"]."' where id_mensaje='".$_POST["id_mensaje"]."'";
    ...
    <?php 
            $conexion=mysqli_connect('localhost','root','root','IglesiaElLugar');
                        ...
                        while($mostrar=mysqli_fetch_array($result)){
                            ?>
            <div>
            <form action="modificarmensaje.php" method="POST">
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  <div class="modal-header text-center">
                    <h4 class="modal-title w-100 font-weight-bold">Modificar mensaje</h4>
                  </div>
                  <div class="modal-body mx-3">
                    ...
                    <?php 
                        $query="SELECT *,concat(nombre, ' ', apellido1) as nombrePredicador from persona where id_permiso=1";
                        $resultado=mysqli_query($conexion,$query);

                        while($fila=mysqli_fetch_array($resultado)){
                            if ($fila['cedula']==$mostrar['id_miembro']) {
                                echo '<option value="'.$fila['cedula'].'" selected>'.$fila['nombrePredicador'].'</option>';
                            }
                            else{
                                echo '<option value="'.$fila['cedula'].'">'.$fila['nombrePredicador'].'</option>';
                            }
                    }
                     ?>
                     </select>
                    ...
                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='id_mensaje' name='id_mensaje' class='form-control validate' placeholder='Código' value='".$mostrar['id_mensaje']."' readonly>";?>
                    </div>

                    <div class="md-form mb-4">
                        <?php echo "<input type='text' id='titulo' name='titulo' class='form-control validate' placeholder='Título del mensaje' value='".$mostrar['nombre']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                      <?php echo "<input type='date' id='fecha' name='fecha' class='form-control validate' placeholder='Fecha' value='".$mostrar['fecha']."'>";?>
                    </div>

                    <div class="md-form mb-4">
                      <?php echo "<input type='text' id='link_pdf' name='link_pdf' class='form-control validate' placeholder='Link del PDF' value='".$mostrar['link_pdf']."'>";?>
                    </div>
